	...
	while(1){
		...
		while(1){
			...
			case WaterCmd_SetMeterNumber:		// " 设置表号 ";
				...
				// 若读取到版本号，则设置表号
				if(true == Protol8009Tranceiver(WaterCmd_ReadMeterCfgInfo, &Addrs, &Args, ackLen, timeout, tryCnt)){
					i = 0;
					Args.buf[i++] = 0x07;		// 命令字	07
					ackLen = 1;					// 应答长度 1	
					// 数据域
					...
					if(true == Protol8009Tranceiver(CurrCmd, &Addrs, &Args, ackLen, timeout, tryCnt)){
